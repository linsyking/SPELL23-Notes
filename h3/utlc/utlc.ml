...
struct
  ...
  let exec_step f e =
    let rec loop i e =
      if i < 10000 then match f e with None -> e | Some e' -> loop (i + 1) e'
      else (
  ...
  let top eval_strat cmds =
    let eval e =
      ...
      | "norm" -> Dynamics.norm e
      | "step" -> exec_step Dynamics.step e
    ...
    Core.List.iter cmds ~f:(function
      ...
      | Syntax.Print e -> print_endline @@ S.to_string e
      | Syntax.Test (e1, e2) ->
          print_endline @@ "Testing e1 = " ^ S.to_string e1;
          print_endline @@ "        e2 = " ^ S.to_string e2;
          print_endline @@ "        e1 = e2 : "
          ^ Bool.to_string (S.aequiv (eval e1) (eval e2)))
end

module LangAst =
  MakeLang (Syntax.Ast) (Syntax.Parsing.Ast) (Language.Dynamics.Ast)

module LangAbt =
  MakeLang (Syntax.Abt) (Syntax.Parsing.Abt) (Language.Dynamics.Abt)
...
let _ =
  let speclist =
    [
      ( "-eval",
        Arg.Symbol ([ "norm"; "step" ], fun s -> eval_strat := s),
        "Evaluation strategy." );
      ( "-rep",
        Arg.Symbol ([ "ast"; "abt" ], fun s -> representation := s),
