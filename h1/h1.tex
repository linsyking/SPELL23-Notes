...
Let's prove their equivalency by using circular implication: DNE $\Rightarrow$ LEM $\Rightarrow$ A3 $\Rightarrow$ DNE.
Here ``$A \Rightarrow B$'' means $\Gamma \vdash_{IPL\cup \{A\}} P\true$ iff $\Gamma \vdash_{IPL\cup \{B\}} P\true$. $\vdash_{IPL}$ means that the derivation is under IPL rules.
